...
describe('createNet — channel fan-out', () => {
  ...
  it('keeps the match channels clear of the rematch protocol', async () => {
    // net.channel() fans out, so 'bid'/'snap' colliding with rematch.ts's
    // 'rv'/'rs'/'rq' would feed every message to both subsystems.
    const { CH_BID, CH_SNAP } = await import('../src/match');
    expect([CH_BID, CH_SNAP]).not.toContain('rv');
    expect([CH_BID, CH_SNAP]).not.toContain('rs');
    expect([CH_BID, CH_SNAP]).not.toContain('rq');
    for (const c of [CH_BID, CH_SNAP]) expect(c.length).toBeLessThanOrEqual(12);
  });
});

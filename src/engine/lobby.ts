...
export function createLobby(config: LobbyConfig): { destroy: () => void } {
  ...
  async function share(): Promise<void> {
    ...
    const shareData = { title: 'Join my Bidstorm table', text: `Room ${config.roomCode}`, url: link };
    if (navigator.share) {
      try {
          ...
          ${
            net.isHost()
              ? `<button class="lobby-btn lobby-start" type="button" ${s.canStart ? '' : 'disabled'}>
                   ${ps.length < minPlayers ? `Waiting for ${minPlayers - ps.length} more…` : 'Deal now'}
                 </button>`
              : `<p class="lobby-wait"><span class="spinner sm" aria-hidden="true"></span> Waiting for the host to deal…</p>`
          }
  ...
  // promoted peer learns the Deal button is now theirs.

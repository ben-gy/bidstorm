 ...
 * authoritative game state and broadcasts snapshots; clients send inputs.
 *
 * Copied from the gh-game-factory patterns/ engine — do not re-roll the
 * peer/host logic here.
 *
 * Trystero limits to remember:
 *  - Action names (channels) must be <= 12 bytes. Keep them short: 'bid','snap'.
...
export interface Net {
   ...
   * receivers and both fire. Use `send.off()` to detach one receiver without
